...
def cwt(u, fs, ns=3, nt=0.25, vpo=10, freqmin=0.02, freqmax=0.1, nptsfreq=200, smooth=True):
    ...
    cwt, scales, freqs, coi, fft, fftfreqs = wavelet.cwt(x, dt, dj, s0, J, mother, freqs=freqlim)
...
def myxwt(trace1, trace2, fs, ns, nt, vpo, freqmin, freqmax, nptsfreq, smooth=False):
    # Choosing a Morlet wavelet with a central frequency w0 = 6
    mother = wavelet.Morlet(6.)
    # nx represent the number of element in the trace_current array
    nx = np.min([np.size(trace1), np.size(trace2)])
    x1 = np.transpose(trace1)
    x2 = np.transpose(trace2)
    # Sampling interval
    dt = 1 / fs
    # Spacing between discrete scales, the default value is 1/12
    dj = 1 / vpo
    ...
    # fft : Normalized fast Fourier transform of the input trace
    # fftfreqs : Fourier frequencies for the calculated FFT spectrum.
    cwt1, scales, freqs, coi, fft, fftfreqs = wavelet.cwt(x1, dt, dj, s0, J, mother, freqs=freqlim)
    cwt2, _, _, _, _, _ = wavelet.cwt(x2, dt, dj, s0, J, mother, freqs=freqlim)
    
    # Cross-wavelet transform operation
    crossCFS = cwt1 * np.conj(cwt2)
